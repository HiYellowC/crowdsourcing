require.config({
	...
	paths : {
		...
		webuploader : 'static/js/lib/webuploader/webuploader'
	}
});
require(
		[ 'jquery', 'webuploader' ],
		function($, WebUploader) {
			...
			jQuery(function() {
				var $ = jQuery, // just in case. Make sure it's not an other
								// libaray.
				...
				uploader = WebUploader.create({
					...
					// 是否分片处理大文件上传
					// chunked: true,
					server : 'http://localhost:8080/image/save',
					fileNumLimit : 300,
				...
				function addFile(file) {
					...
					file.on('statuschange', function(cur, prev) {
						...
						if (cur === 'error' || cur === 'invalid') {
							file.isDuplicated ? showError('duplicate') : showError(file.statusText);
